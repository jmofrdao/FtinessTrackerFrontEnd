...
const AddRoutine = ({ routines, setRoutines }) => {
  ...
  async function handleSubmit(event) {
    ...
    const freshRoutine = await addNewRoutine(token, name, goal, isPublic);
  ...
  return (
    <div>
      ...
      {error && error.message ? (
        <h3>Cannot create a Routine with a name that already exists.</h3>
      ) : null}
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" id="addRoutineButton">
          CREATE
        </button>
      </form>
    </div>
